...
class MovieView(Resource):
    ...
    def put(self, movie_id):
        db.session.query(model.Movie).filter(model.Movie.id == movie_id).update(request.json)
        db.session.commit()
        return None, 200

    def delete(self, movie_id):
        db.session.query(model.Movie).filter(model.Movie.id == movie_id).delete()
...
class MoviesView(Resource):
    ...
    def post(self):
        ...
        db.session.add(model.Movie(**movie))
        db.session.commit()
        return None, 201
...
class DirectorView(Resource):
    def get(self, director_id):
        director = db.session.query(model.Director).filter(model.Director.id == director_id).first()
        if director is None:
            return None, 404
...
class DirectorsView(Resource):
    def get(self):
        directors = db.session.query(model.Director).all()
...
class GenreView(Resource):
    def get(self, genre_id):
        genre = db.session.query(model.Genre).filter(model.Genre.id == genre_id).first()
        if genre is None:
            return None, 404
